...
- [OpenClaw API Documentation](https://docs.anthropic.com/en/api/openclaw) — Полная API-справка для управления agents, tasks, goals, tools, schedules.
- [Anthropic SDK (Python)](https://github.com/anthropics/anthropic-sdk-python) — Официальный SDK для Python с поддержкой OpenClaw agents.
- [Anthropic SDK (TypeScript)](https://github.com/anthropics/anthropic-sdk-js) — Официальный SDK для Node.js / TypeScript.
- [Anthropic SDK (Go)](https://github.com/anthropics/anthropic-sdk-go) — Официальный SDK для Go.
- [OpenClaw Cookbook](https://github.com/anthropics/anthropic-cookbook) — Примеры паттернов: automated workflows, parallel agents, budget optimization.
- [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) — Стандарт подключения tools и resources. Поддерживается OpenClaw agentsами.
- [awesome-openclaw-skills](https://sky-lv.github.io/awesome-openclaw-skills) — Коллекция готовых skills для OpenClaw agents.

### Туториалы и примеры

- [Getting Started with OpenClaw](https://docs.anthropic.com/en/docs/openclaw/getting-started) — Official quickstart guide для первой managed-task.
- [Building Declarative Workflows](https://docs.anthropic.com/en/docs/openclaw/workflows) — Как описать multi-step workflow через task definitions вместо prompt.
- [Parallel Agent Execution](https://docs.anthropic.com/en/docs/openclaw/parallel-agents) — Запуск 100+ агентов параллельно, coordination через shared context.
- [Budget Control and Cost Tracking](https://docs.anthropic.com/en/docs/openclaw/budgets) — Настройка per-task limits, audit logs затрат, оптимизация tokens/cost ratio.
- [Scheduling and Cron Tasks](https://docs.anthropic.com/en/docs/openclaw/schedules) — Периодические задачи: ночной ревью, ежедневная синхронизация, фоновые процессы.
- [Connecting MCP Tools](https://docs.anthropic.com/en/docs/openclaw/mcp-tools) — Подключение GitHub, Slack, databases и custom HTTP endpoints к agentsам.
- [Webhooks and Notifications](https://docs.anthropic.com/en/docs/openclaw/webhooks) — Callback на completion, error, timeout — интеграция с вашей системой.
- [Multi-Team and Enterprise Setup](https://docs.anthropic.com/en/docs/openclaw/enterprise) — RBAC, audit logs, dedicated infrastructure, SLA. Enterprise-only.
